...
class UNET(nn.Module):
    def __init__(
        ...
        # Descending Portion of UNET
        for feature in features:
            self.downs.append(DoubleConv(in_channels, feature))
            in_channels = feature
        
        # Ascending Portion of UNET
    ...
    def forward(self, x):
        ...
        for idx in range(0, len(self.ups), 2): # step by 2
            x = self.ups[idx](x)
            skip_connections = skip_connections[idx//2]
            if x.shape != skip_connections.shape:
                # add padding - may impact accuracy
                x = TF.resize(x, size=skip_connections[2:])
